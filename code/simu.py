...
deltas = np.linspace(0.1, 1.5, 15)

for delta_id in range(15):
    
    delta = deltas[delta_id - 1]
    rho = delta**2 /2 
    ...
    for j in range(3):
        ...
        inf_low[cidx_list[ifold(j+2,3)]] = adj_h0 / (1-pp)
    ...
    for j in range(3):
        ...
        inf_upp[cidx_list[ifold(j+2,3)]] = adj_h0 / (1-pp)
    ...
    to_save = pd.DataFrame({"inf_low": inf_low, "inf_upp": inf_upp, "T": data["T"], 
    ...
    to_save.to_csv(os.path.join(save_dir, "data_"+str(seed)+"_delta_"+str(delta_id)+".csv"))
    summary.to_csv(os.path.join(save_dir, "sum_"+str(seed)+"_delta_"+str(delta_id)+".csv"))
